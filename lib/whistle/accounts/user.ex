defmodule Whistle.Accounts.User do
  use Ecto.Schema
  ...
  @doc """
  A user changeset for changing the club. This is mostly
  needed after a user has registered.
  """
  def club_changeset(user, attrs) do
    user
    |> cast(attrs, [:club_id])
  end

  @doc """
  A user changeset for changing the role and profile information.
  ...
  """
  def valid_password?(%Whistle.Accounts.User{hashed_password: hashed_password}, password)
  ...
  defp validate_role(changeset) do
    ...
    |> validate_inclusion(:role, Role.roles(),
      message: "must be one of: #{Enum.join(Role.roles(), ", ")}"
    )
  end
end
