defmodule WhistleWeb.UserAuth do
  use WhistleWeb, :verified_routes
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      WhistleWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def assign_current_path(conn, _opts) do
  ...
  ## `on_mount` arguments
    ...
    * `:ensure_authenticated` - Authenticates the user from the session,
      and assigns the current_user. Redirects to login page if there's
      no logged user.
  ...
  the current_user:
      ...
      defmodule WhistleWeb.PageLive do
        use WhistleWeb, :live_view

        on_mount {WhistleWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{WhistleWeb.UserAuth, :ensure_authenticated}] do
  ...
  @doc """
  Used for routes that require the user to have a club selected.
  """
  def require_club(conn, _opts) do
    if conn.assigns[:current_user] && conn.assigns[:current_user].club_id do
      conn
    else
      conn
      |> put_flash(:error, "Bitte wähle zuerst einen Verein aus.")
