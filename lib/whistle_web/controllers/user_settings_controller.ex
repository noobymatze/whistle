defmodule WhistleWeb.UserSettingsController do
  ...
  def confirm_email(conn, %{"token" => token}) do
    case Accounts.update_user_email(conn.assigns.current_user, token) do
      :ok ->
        conn
        |> put_flash(:info, "E-Mail erfolgreich geändert.")
        |> redirect(to: ~p"/users/settings")

      :error ->
        conn
        |> put_flash(:error, "Der Link zur E-Mail-Änderung ist ungültig oder abgelaufen.")
        |> redirect(to: ~p"/users/settings")
    end
  end
