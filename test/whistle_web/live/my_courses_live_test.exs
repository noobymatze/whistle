defmodule WhistleWeb.MyCoursesLiveTest do
  ...
  defp mandatory_date_fixture(course, attrs) do
    {:ok, date} =
      Courses.create_course_date(
        Map.merge(
          %{course_id: course.id, date: ~D[2099-05-13], time: ~T[14:00:00], kind: :mandatory},
          attrs
        )
      )
  ...
  describe "my courses online rescheduling" do
    setup do
      ...
      mandatory1 = mandatory_date_fixture(course, %{date: ~D[2099-05-13], time: ~T[14:00:00]})
      mandatory2 = mandatory_date_fixture(course, %{date: ~D[2099-05-14], time: ~T[15:00:00]})

      elective1 =
        elective_date_fixture(course, topic, %{date: ~D[2099-05-21], time: ~T[16:00:00]})

      elective2 =
        elective_date_fixture(course, topic, %{date: ~D[2099-05-22], time: ~T[18:00:00]})
    ...
    test "user can reschedule only the mandatory date", %{
      ...
      html = render(lv)
      assert html =~ "14.05.2099"
      assert html =~ "21.05.2099"
    ...
    test "forged save_reschedule for another registration is rejected", %{
      ...
      other_mandatory =
        mandatory_date_fixture(other_course, %{date: ~D[2099-05-16], time: ~T[12:00:00]})
    ...
    test "shows an error flash when the target date is full", %{
      ...
      full_mandatory = mandatory_date_fixture(course, %{date: ~D[2099-05-17], time: ~T[09:00:00]})

      target_mandatory =
        mandatory_date_fixture(course, %{date: ~D[2099-05-18], time: ~T[10:00:00]})

      occupying_elective =
        elective_date_fixture(course, topic, %{date: ~D[2099-05-25], time: ~T[11:00:00]})

      own_elective =
        elective_date_fixture(course, topic, %{date: ~D[2099-05-26], time: ~T[12:00:00]})
